...
import { getTeamMembership, requireTeamRole } from "@/modules/core/permissions";
...
describe("joinTeam", () => {
  ...
  it("绕过查重的唯一键冲突（23505）被转译为可展示的 AppError", async () => {
    ...
    const tx = db.transaction(async (trx) => {
      await trx
        .insert(teamMembers)
        .values({ teamId: team.id, userId: joiner.id, role: "student" });
      markInserted();
      await hold;
    });
...
describe("requireTeamRole", () => {
  ...
  it("非成员抛 ForbiddenError", async () => {
    ...
    const team = await createTeam(owner.id, "东吴实验室");
    await expect(
      requireTeamRole(outsider.id, team.id, ["admin", "teacher", "student"]),
  ...
  it("角色不足抛 ForbiddenError", async () => {
    ...
    await expect(requireTeamRole(student.id, team.id, ["admin"])).rejects.toThrow(
      "没有权限",
    );
  });
  ...
  it("teacher 矩阵：允许列表含 teacher 则通过，仅 admin 则拒", async () => {
    ...
    await expect(requireTeamRole(teacher.id, team.id, ["admin"])).rejects.toThrow(
      "没有权限",
    );
  });
});
...
describe("updateMemberRole", () => {
  ...
  it("student 无权改角色", async () => {
    ...
    await expect(updateMemberRole(s1.id, team.id, s2.id, "teacher")).rejects.toThrow(
      "没有权限",
    );
  });
  ...
  it("目标用户不在团队中抛可展示错误", async () => {
    ...
    await expect(
      updateMemberRole(owner.id, team.id, outsider.id, "teacher"),
